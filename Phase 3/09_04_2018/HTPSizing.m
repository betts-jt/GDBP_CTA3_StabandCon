function [Shtp] = HTPSizing(plotscissor, MAC, h0, a, ahtpC, ClmaxL, lhtp, deda, Cm0, xNLG, xMLG, hNLG, hMLG, Ct, zt, rho, Vmc, S, MTOW, cgRange, CltMin)
...
%Xcg = [XcgFWD-0.5:0.1:XcgAFT+0.5]; %To use after implementing data base
...
SRatioLand = ((Cm0-(h0-h)*ClmaxL)*MAC)/(lhtp*CltMin); % St/S ratio based on Landing Cltmin
SRatioTO = ((Cm0+(h0-h)*ClmaxL)+Ct*(zt/MAC)-((((MTOW*9.81)/(q*S))-ClmaxL)*(hMLG-h)))./((-CltMin*(hMLG-h))-(CltMin*(-(h0-h)+(lhtp/MAC)))); % St/S ratio based on TO Rotation
