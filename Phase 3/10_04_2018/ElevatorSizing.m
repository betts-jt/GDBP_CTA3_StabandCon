...
if ESizingMethod ==0
    ...
    Lwf = 0.5*data('rho')*(data('Vmc')*1.05)^2*Clto*data('S'); % Wing Fusalage Lift
    Lhtp = 0.5*data('rho')*(data('Vmc')*1.05)^2*data('CltMin')*data('Shtp');
    Dto = 0.5*data('rho')*(data('Vmc')*1.05)^2*Cdto*data('S'); % TO Drag
    Mac = 0.5*data('rho')*(data('Vmc')*1.05)^2*data('Cm0')*data('S')*data('MAC'); %Pitching moment about aerodynamic center
    ...
    Iyy = (data('b')^2*data('MTOW')*data('Rybar')^2)/(4*9.81); %Initial class 1 calculation of moment of intetia using radius of gyration
    
    Lh = (Mlwf+Mac+Ma-Mw+Md-Mt-(Iyy*(PitchAngAcc)))/((data('noseTo25MAC')+data('lhtp'))-data('xMLG')); % Required tailplane lift for rotation
    
    Clh1 = Lh/(0.5*data('rho')*(data('Vmc')*1.05)^2*data('Shtp')); % Required tailplane lift coefficient to rotation
    Clh1 = -Clh1;
    ...
    Clh2 = LiftingLineTheory(PlotLiftDis, data('ARh'), data('bh'), delta_a_0, data('TaperRh'), data('ahtp2D'), data('bebh'), data('etaT'), data('MACh'));
