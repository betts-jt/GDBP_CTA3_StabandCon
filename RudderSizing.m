function [Sr, rHinge] = RudderSizing(data, Svtp, CDoei, RSizingMethod)
...
elseif RSizingMethod == 1
    ...
    NA = (-data('TRot')-(CDoei*0.5*data('rhoSL')*(data('Vmc')*1.05)^2*data('S')))*data('yt'); % Max required yawing moment for directional control
    
    Cndr = (-NA)/(-0.5*data('rhoSL')*(data('Vmc')*1.05)^2*data('S')*data('b')*rudderDefMaxR); % Rudder control derrivative
