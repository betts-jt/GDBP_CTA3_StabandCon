...
Xcg = [12:0.05:14.5]; % range of cg positions. m
...
if plotscissor == 1
    ...
    plot(h, SRatioLand, '--'); % Plotting limiting lime to to min landing
    plot(h, SRatioTO, '-.'); % Plotting limiting lime to to min take off rotation
    plot(hAftNLGR, StSAftNLG, '-'); % Plotting limiting lime to to min weight on NLG
    ...
    plot(h, SRatioTO, '-.'); % Plotting limiting lime to to min take off rotation
    plot(hAftNLGR, StSAftNLG, '-'); % Plotting limiting lime to to min weight on NLG
    plot([aa hAftNLG], [y y]); % Plot horizontal line indicating the specified CG range
    legend('Kn', 'Landing', 'Take Off', 'Nose Landing Gear','Location','southwest');
