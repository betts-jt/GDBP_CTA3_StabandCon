...
importVar = importData(workbookFile, 'Sheet1', 'A2:C139');
...
[Svtp, CDoei] = VTPSizing(plotVtpGraphs, data, data('Cyb'), data('Cyvb'), data('Cnb'), data('Cyvr'));
[Cyb, Cybv, Cnb, Cydr] = RudConDer(data, Svtp, Shtp);
[SvtpN, CDoei] = VTPSizing(plotVtpGraphs, data, Cyb, Cybv, Cnb, Cydr);

loop = 1; %Loop variable for derrivative convnergance loop

while loop==1
    if (0.9*Svtp < SvtpN)&&(SvtpN < 1.1*Svtp)
        loop = 0
    else
        Svtp = SvtpN
        [Cyb, Cybv, Cnb, Cydr] = RudConDer(data, Svtp, Shtp);
